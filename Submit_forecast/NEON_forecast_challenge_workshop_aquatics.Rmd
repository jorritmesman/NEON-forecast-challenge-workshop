...
if("eastward_wind" %in% variables & "northward_wind" %in% variables){
  # Calculate wind speed from u and v wind
  noaa_past_wind = noaa_past[noaa_past$variable == "eastward_wind",]
  noaa_past_wind$variable = "wind_speed"
  noaa_past_wind$prediction = sqrt(noaa_past[noaa_past$variable == "eastward_wind", "prediction"][[1L]]^2 +
                                     noaa_past[noaa_past$variable == "northward_wind", "prediction"][[1L]]^2)
  
  noaa_past = noaa_past[noaa_past$variable == "air_temperature",]
  noaa_past = rbind(noaa_past, noaa_past_wind)
}
...
noaa_future_s3 <- neon4cast::noaa_stage2()
variables <- c("air_temperature", "eastward_wind", "northward_wind")

noaa_future <- noaa_future_s3 |> 
  dplyr::filter(start_date == as.character(noaa_date), # This was my addition to avoid error with noaa_stage2 function
...
if("eastward_wind" %in% variables & "northward_wind" %in% variables){
  # Calculate wind speed from u and v wind
  noaa_future_wind = noaa_future[noaa_future$variable == "eastward_wind",]
  noaa_future_wind$variable = "wind_speed"
  noaa_future_wind$prediction = sqrt(noaa_future[noaa_future$variable == "eastward_wind", "prediction"][[1L]]^2 +
                                     noaa_future[noaa_future$variable == "northward_wind", "prediction"][[1L]]^2)
  
  noaa_future = noaa_future[noaa_future$variable == "air_temperature",]
  noaa_future = rbind(noaa_future, noaa_future_wind)
}
...
noaa_future_daily <- noaa_future |> 
  ...
  # convert to Celsius
  mutate(air_temperature = air_temperature - 273.15) |> 
  select(datetime, site_id, air_temperature, wind_speed, parameter)
...
# Wind speed plots
ggplot(noaa_future_daily, aes(x=datetime, y=wind_speed)) +
  geom_line(aes(group = parameter), alpha = 0.4)+
  geom_line(data = noaa_past_mean, colour = 'darkblue') +
  facet_wrap(~site_id, scales = 'free')
ggplot(noaa_future_daily, aes(x=datetime, y=wind_speed)) +
...
# Jorrit: I just added the effect of wind speed, even though it's not significant
forecasted_temperature <- fit$coefficients[1] + fit$coefficients[2] * noaa_future_site$air_temperature +
  fit$coefficients[3] * noaa_future_site$wind_speed
...
for(i in 1:length(lake_sites$field_site_id)) {  
  ...
  # This would be where you can modify the forecast
...
my_model_id <- 'JorritsCrystalBall'
